...
public class TypeCheckVisitor extends TypeDepthFirstVisitor {
	...
	public Type visit(Assign n) {
		...
		{
			if (!(n.e instanceof SeqExp || n.e instanceof NewArray))
			{
				error.complain("Type mismatch in assignment of '" + n.i.toString() + "' " + n.position.toString() + ". Array assignment must be a new array or a seq().");
			}
		}
		else {
			if (varType == null || expType == null
	...
	public Type visit(SeqExp n) {
		Type t1 = n.e1.accept(this);
		Type t2 = n.e2.accept(this);
		
		if (!(t1 instanceof IntegerType)) {
			error.complain("Sequence lower bound must evaluate to an integer." + n.position.toString());
		}
		if (!(t2 instanceof IntegerType)) {
			error.complain("Sequence upper bound must evaluate to an integer." + n.position.toString());
		}
		return new IntArrayType(); //TODO: is this right?
	}
